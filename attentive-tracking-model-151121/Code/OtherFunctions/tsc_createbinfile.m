...
% In the TASCAR scene the monaural file is placed as a sound source
...
% load TASCAR matlab tools  
javaaddpath netutil-1.0.0.jar
addpath /usr/share/tascar/matlab/

% transform angle trajectory to cartesian coordinates
% and save in a csv file to be later loaded into tascar
...
% remove the resulting audio file
% system(['rm ', outputfilename]);

% system(['rm ', strcat(b,'/',[sStimParam.runtag,'.tsc'])]);

%set the duration of the scene so that it equals the duration of stimulus
% tsc_filename=[heredir,'moving_speaker_',sStimParam.render,'.tsc'];
% doc = tascar_xml_open(tsc_filename);
% doc = tascar_xml_edit_elements( doc, 'session', 'duration',sStimParam.dur*0.001);
% tascar_xml_save( doc, tsc_filename )c
